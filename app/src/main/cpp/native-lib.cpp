...
Java_com_mgg_variant_MainActivity_stringFromJNI(JNIEnv *env,
                                                jobject /* this */) {
    mpark::variant<std::string> temp;
    temp = "hhh";
...
template<typename L>
std::ostream &write_bracketed_list(std::ostream &os, const L &vec, char left,
                                   char right) {
    os << left;
    if (!vec.empty()) {
        auto &&it = vec.begin();
...
Data::~Data() { JAVA_OUTPUT << "destroyed" << std::endl; }
...
private:
    Sample(const Sample &) =
    default;  // ensure that external code cannot make copies

    Data _data;
};

Sample::Sample() { JAVA_OUTPUT << "created" << std::endl; }
...
Sample::~Sample() { JAVA_OUTPUT << "destroyed" << std::endl; }
...
float returns_float() { return std::numeric_limits<float>::max(); }

double returns_double() { return std::numeric_limits<double>::max(); }

std::string returns_string() { return "a sample string"; }

bool pass_arguments_by_value(std::string str, bool b, short s, int i, long l,
                             int16_t i16, int32_t i32, int64_t i64, float f,
                             double d) {
    JAVA_OUTPUT << "("
                << "string = " << str << ", "
                << "bool = " << b << ", "
...
bool pass_arguments_by_reference(const std::string &str, const bool &b,
                                 const short &s, const int &i, const long &l,
                                 const int16_t &i16, const int32_t &i32,
                                 const int64_t &i64, const float &f,
                                 const double &d) {
    JAVA_OUTPUT << "("
                << "string = " << str << ", "
                << "bool = " << b << ", "
...
std::vector<unsigned char> array_of_char(
...
std::map<std::string, std::string> ordered_map_of_string(
...
std::map<std::string, std::vector<std::string>> native_composite(
...
void pass_callback(std::function<void()> fun) { fun(); }

std::string pass_callback_returns_string(std::function<std::string()> fun) {
    return fun();
}

int pass_callback_string_returns_int(std::string str,
                                     std::function<int(std::string)> fun) {
    return fun(str);
}

std::string pass_callback_string_returns_string(
        std::string str, std::function<std::string(std::string)> fun) {
    return fun(str);
}

std::string pass_callback_arguments(
        std::string str,
        std::function<std::string(std::string, short, int, long)> fun) {
    return fun(str, 4, 82, 112);
}

void callback_on_native_thread(std::function<void()> fun) {
    std::thread([fun]() { fun(); }).join();
}

void raise_native_exception() { throw std::runtime_error("an expected error"); }
...
JAVA_EXTENSION_MODULE() {
    ...
    native_class<Sample>()
            ...
            .function<static_cast<void (Sample::*)(const Data &)>(&Sample::set_data)>(
                    "set_data")
            ...
            .function<pass_callback_string_returns_int>(
                    "pass_callback_string_returns_int")
            .function<pass_callback_string_returns_string>(
                    "pass_callback_string_returns_string")
