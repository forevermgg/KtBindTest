//
// Created by 蒙刚刚 on 2021/9/18.
// https://github.com/Kheiron-Medical/ktbind/blob/master/cpp/include/ktbind/ktbind.hpp
//
#pragma once
 ...
 * @tparam I An index sequence, typically constructed with
...
/**
 * Replaces all occurrences of a character in a string with another character at
 * compile time.
...
/**
 * Concatenates a list of strings at compile time, inserting a separator between
 * neighboring items.
 */
template <std::string_view const &Sep, std::string_view const &...Items>
struct join_sep;

template <std::string_view const &Sep, std::string_view const &Head,
          std::string_view const &...Tail>
struct join_sep<Sep, Head, Tail...> {
  constexpr static std::string_view value =
      join<Head, join<Sep, Tail>::value...>::value;
};
...
struct JavaException : std::exception {
  JavaException(JNIEnv *env) {
    if (env->ExceptionCheck()) {
      ...
      jmethodID getMessageFunc = env->GetMethodID(exceptionClass, "getMessage",
                                                  "()Ljava/lang/String;");
      jstring messageObject =
          static_cast<jstring>(env->CallObjectMethod(ex, getMessageFunc));
  ...
  /**
   * Used by the interoperability framework to re-throw the exception in Java
   * before crossing the native to Java boundary, unless the exception has been
   * caught by the user.
   */
  jthrowable innerException() const noexcept { return ex; }
...
class Method {
 ...
 public:
  ...
  jmethodID ref() const { return _ref; }
};
...
class Field {
 ...
 public:
  ...
  jfieldID ref() const { return _ref; }
};
...
/**
 * Scoped C++ wrapper class of a [jobject] that is used only within a single
...
class LocalClassRef {
 public:
  ...
  LocalClassRef(JNIEnv *env, const char *name, std::nothrow_t) : _env(env) {
    ...
    class_name.assign(name);
    if (startsWith(class_name, "L") && endsWith(class_name, ";")) {
      class_name = class_name.substr(1, class_name.length() - 2);
    }
    _ref = env->FindClass(class_name.c_str());
  }
    ...
    if (_env == nullptr) {
      ...
      switch (_vm->GetEnv(reinterpret_cast<void **>(&_env), JNI_VERSION_1_6)) {
        case JNI_OK:
          break;
        case JNI_EDETACHED:
          if (_vm->AttachCurrentThread(reinterpret_cast<JNIEnv **>(&_env),
                                       nullptr) == JNI_OK) {
            assert(_env != nullptr);
            _attached = true;
          } else {
...
/**
 * Ensures that Java resources allocated by the thread are released when the
 * thread terminates.
 */
static thread_local Environment this_thread;

/**
 * An adapter for an object reference handle that remains valid as the
 * native-to-Java boundary is crossed.
 */
class GlobalObjectRef {
 public:
  ...
  GlobalObjectRef(JNIEnv *env, jobject obj) {
    _ref = std::shared_ptr<jobject_struct>(env->NewGlobalRef(obj),
                                           [](jobject ref) {
  ...
  jobject ref() const { return _ref.get(); }
...
/**
 * Used in static_assert to have the type name printed in the compiler error
 * message.
 */
template <typename>
 ...
 *
 * Template substitution fails automatically unless the type is a well-known
 * type or has been declared with DECLARE_DATA_CLASS or DECLARE_NATIVE_CLASS.
 */
template <typename T>
struct ArgType {
  ...
  constexpr static std::string_view array_type_sig =
      join_v<array_type_prefix, ArgType<T>::type_sig>;
  ...
  static_assert(fail<T>::value,
                "Unrecognized type detected, ensure that all involved types "
                "have been declared as a binding type with DECLARE_DATA_CLASS "
                "or DECLARE_NATIVE_CLASS.");
};
...
struct FundamentalArgType {
 ...
 private:
  ...
  /** Used in looking up the appropriate `valueOf()` function to instantiate
   * object wrappers of values of a primitive type. */
  constexpr static std::string_view value_initializer =
      join_v<lparen, ArgType<T>::type_sig, rparen, class_type_prefix,
             ArgType<T>::class_name, class_type_suffix>;

  /** Used in looking up the appropriate `primitiveValue()` function to fetch
   * the primitive type (e.g. `intValue()` for `int`). */
  constexpr static std::string_view get_value_func_suffix = "Value";
  constexpr static std::string_view get_value_func =
      join_v<ArgType<T>::primitive_type, get_value_func_suffix>;
  constexpr static std::string_view get_value_func_sig =
      join_v<lparen, rparen, ArgType<T>::type_sig>;

 public:
  static T native_value(JNIEnv *, J value) { return static_cast<T>(value); }

  static J java_value(JNIEnv *env, T value) { return static_cast<J>(value); }
  ...
  static T native_field_value(JNIEnv *env, jobject obj, Field &fld) {
    return ArgType<T>::native_value(
        env, ArgType<T>::java_raw_field_value(env, obj, fld));
  }
   ...
   */
  static jarray java_array_value(JNIEnv *env, const native_type *ptr,
                                 std::size_t len);
};
...
struct ArgType<bool> : FundamentalArgType<bool, jboolean> {
  ...
  static void java_field_value(JNIEnv *env, jobject obj, Field &fld,
                               native_type value) {
    env->SetBooleanField(obj, fld.ref(), java_value(env, value));
  }

  static void native_array_value(JNIEnv *env, jarray arr, native_type *ptr,
                                 std::size_t len) {
    static_assert(sizeof(native_type) == sizeof(java_type),
                  "C++ boolean type and JNI jboolean type are expected to "
                  "match in size.");
    env->GetBooleanArrayRegion(static_cast<jbooleanArray>(arr), 0, len,
                               reinterpret_cast<jboolean *>(ptr));
  }
  ...
  static jarray java_array_value(JNIEnv *env, const native_type *ptr,
    ...
    env->SetBooleanArrayRegion(arr, 0, len,
                               reinterpret_cast<const jboolean *>(ptr));
    return arr;
  }
};
...
struct CharArgType : FundamentalArgType<T, jbyte> {
  ...
  static void native_array_value(JNIEnv *env, jarray arr, T *ptr,
                                 std::size_t len) {
    env->GetByteArrayRegion(static_cast<jbyteArray>(arr), 0, len,
                            reinterpret_cast<jbyte *>(ptr));
  }
...
template <>
struct ArgType<char> : CharArgType<char> {};
template <>
struct ArgType<signed char> : CharArgType<signed char> {};
template <>
struct ArgType<unsigned char> : CharArgType<unsigned char> {};
...
struct Int32ArgType : FundamentalArgType<T, jint> {
  ...
  static void native_array_value(JNIEnv *env, jarray arr, T *ptr,
    ...
    static_assert(sizeof(T) == sizeof(jint),
                  "C++ and JNI integer types are expected to match in size.");
    env->GetIntArrayRegion(static_cast<jintArray>(arr), 0, len,
...
struct Int64ArgType : FundamentalArgType<T, jlong> {
  ...
  static void native_array_value(JNIEnv *env, jarray arr, T *ptr,
    ...
    static_assert(sizeof(T) == sizeof(jlong),
                  "C++ and JNI integer types are expected to match in size.");
    env->GetLongArrayRegion(static_cast<jlongArray>(arr), 0, len,
...
template <typename T>
struct IntegerArgType
    : std::conditional<sizeof(T) <= sizeof(int32_t), Int32ArgType<T>,
                       Int64ArgType<T>>::type {};

template <>
struct ArgType<int> : IntegerArgType<int> {};
template <>
struct ArgType<long> : IntegerArgType<long> {};
template <>
struct ArgType<long long> : IntegerArgType<long long> {};
template <>
struct ArgType<unsigned int> : IntegerArgType<unsigned int> {};
template <>
struct ArgType<unsigned long> : IntegerArgType<unsigned long> {};
template <>
struct ArgType<unsigned long long> : IntegerArgType<unsigned long long> {};
...
struct ArgType<T *> : private FundamentalArgType<std::intptr_t, jlong> {
  ...
  static T *native_field_value(JNIEnv *env, jobject obj, Field &fld) {
    return reinterpret_cast<T *>(
        native_value(env, env->GetLongField(obj, fld.ref())));
  }

  static void java_field_value(JNIEnv *env, jobject obj, Field &fld, T *value) {
    env->SetLongField(obj, fld.ref(),
                      java_value(env, reinterpret_cast<intptr_t>(value)));
  }
};
...
/**
 * Generates a human-readable name for specialized generic types, e.g.
 * List<String>.
 */
template <std::string_view const &template_type,
          std::string_view const &...arg_types>
struct kotlin_type_specialization {
  ...
  constexpr static std::string_view value =
      join_v<template_type, lt, join_sep_v<comma, arg_types...>, gt>;
};
...
struct ArgType<Array<T>> {
 ...
 public:
  constexpr static std::string_view kotlin_type =
      kotlin_type_specialization<array_type, ArgType<T>::kotlin_type>::value;
  constexpr static std::string_view type_sig =
      join_v<array_type_prefix, ArgType<T>::type_sig>;
...
/**
 * Base class for all object types such as String, List<T>, Map<K, V>, data
...
struct CompositeArgType {
 ...
 public:
  ...
  constexpr static std::string_view type_sig =
      join_v<class_type_prefix, ArgType<T>::class_name, class_type_suffix>;
  ...
  static jobject java_box(JNIEnv *env, J value) { return value; }

  static J java_unbox(JNIEnv *env, jobject obj) { return obj; }
};
...
struct ArgType<std::string> : CompositeArgType<std::string, jstring> {
  ...
  static native_type native_field_value(JNIEnv *env, jobject obj, Field &fld) {
    return native_value(
        env, static_cast<jstring>(env->GetObjectField(obj, fld.ref())));
  }
...
struct ListArgType : CompositeArgType<L, jobject> {
  ...
  constexpr static std::string_view kotlin_type =
      kotlin_type_specialization<qualified_name,
                                 ArgType<T>::kotlin_type>::value;
...
struct FundamentalArgArrayType {
 ...
 public:
  constexpr static std::string_view kotlin_type =
      join_v<ArgType<T>::kotlin_type, kotlin_array>;
  constexpr static std::string_view type_sig =
      join_v<array_type_prefix, ArgType<T>::type_sig>;
  ...
  static jarray java_box(JNIEnv *env, jarray value) { return value; }

  static jarray java_unbox(JNIEnv *env, jarray obj) { return obj; }
...
template <>
struct ArgType<std::vector<char>> : FundamentalArgArrayType<char> {};
template <>
struct ArgType<std::vector<signed char>>
    : FundamentalArgArrayType<signed char> {};
template <>
struct ArgType<std::vector<unsigned char>>
    : FundamentalArgArrayType<unsigned char> {};
template <>
struct ArgType<std::vector<uint16_t>> : FundamentalArgArrayType<uint16_t> {};
template <>
struct ArgType<std::vector<short>> : FundamentalArgArrayType<short> {};
template <>
struct ArgType<std::vector<int>> : FundamentalArgArrayType<int> {};
template <>
struct ArgType<std::vector<long>> : FundamentalArgArrayType<long> {};
template <>
struct ArgType<std::vector<long long>> : FundamentalArgArrayType<long long> {};
template <>
struct ArgType<std::vector<unsigned int>>
    : FundamentalArgArrayType<unsigned int> {};
template <>
struct ArgType<std::vector<unsigned long>>
    : FundamentalArgArrayType<unsigned long> {};
template <>
struct ArgType<std::vector<unsigned long long>>
    : FundamentalArgArrayType<unsigned long long> {};
template <>
struct ArgType<std::vector<float>> : FundamentalArgArrayType<float> {};
template <>
struct ArgType<std::vector<double>> : FundamentalArgArrayType<double> {};
...
/**
 * Converts a native dictionary (e.g. a [map] or [unordered_map]) into a Java
 * Map.
 */
...
struct MapArgType : CompositeArgType<M, jobject> {
  ...
  constexpr static std::string_view kotlin_type =
      kotlin_type_specialization<qualified_name, ArgType<K>::kotlin_type,
                                 ArgType<V>::kotlin_type>::value;
...
struct ArgType<std::unordered_map<K, V>>
    : MapArgType<std::unordered_map<K, V>, K, V> {
...
struct FieldBinding {
  ...
  void (*get_by_value)(JNIEnv *env, jobject obj, Field &fld,
                       const void *native_object_ptr);

  /** A function that persists a value to a Java object field. */
  void (*set_by_value)(JNIEnv *env, jobject obj, Field &fld,
                       void *native_object_ptr);
};
...
struct DataClassArgType : CompositeArgType<T, jobject> {
  ...
  constexpr static std::string_view type_sig =
      CompositeArgType<T, jobject>::type_sig;
...
/**
 * Marshals types that live primarily in the native space, and accessed through
 * an opaque handle in Java/Kotlin.
 */
template <typename T>
struct NativeClassArgType : CompositeArgType<T, jobject> {
  ...
  static jobject java_value(JNIEnv *env, T &&native_object) {
    ...
    Field field =
        objClass.getField("nativePointer", ArgType<T *>::type_sig.data());
...
struct Function<R(Args...)> {
 private:
  static_assert(std::is_same<R, typename ArgType<R>::native_type>::value,
                "Unrecognized type detected, ensure that all involved types "
                "have been declared as a binding type with DECLARE_DATA_CLASS "
                "or DECLARE_NATIVE_CLASS.");
  static_assert(
      (std::is_same<Args, typename ArgType<Args>::native_type>::value && ...),
      "Unrecognized type detected, ensure that all involved types have been "
      "declared as a binding type with DECLARE_DATA_CLASS or "
      "DECLARE_NATIVE_CLASS.");

  template <std::string_view const &param_sig,
            std::string_view const &return_sig>
  struct callable_sig {
    constexpr static std::string_view lparen = "(";
    constexpr static std::string_view rparen = ")";
    constexpr static std::string_view value =
        join_v<lparen, param_sig, rparen, return_sig>;
  };
  ...
  struct function_params<std::index_sequence<I...>> {
    ...
    constexpr static std::string_view value =
        join_sep_v<comma, join_v<arg, integer_to_digits<I>::value, colon,
                                 ArgType<Args>::kotlin_type>...>;
  };

  struct lambda_params {
    constexpr static std::string_view comma = ", ";
    constexpr static std::string_view value =
        join_sep_v<comma, ArgType<Args>::kotlin_type...>;
  };

  constexpr static std::string_view param_sig =
      join_v<ArgType<Args>::type_sig...>;
  constexpr static std::string_view return_sig = ArgType<R>::type_sig;

  constexpr static std::string_view colon = ": ";
  constexpr static std::string_view kotlin_params =
      function_params<std::index_sequence_for<Args...>>::value;
  constexpr static std::string_view kotlin_return =
      join_v<colon, ArgType<R>::kotlin_type>;

  constexpr static std::string_view arrow = " -> ";
  constexpr static std::string_view kotlin_lambda_params = lambda_params::value;
  constexpr static std::string_view kotlin_lambda_return =
      join_v<arrow, ArgType<R>::kotlin_type>;

 public:
  /** Java signature string used internally for type lookup. */
  constexpr static std::string_view signature =
      callable_sig<param_sig, return_sig>::value;

  /** Human-readable type definition for use as a member function. */
  constexpr static std::string_view kotlin_type =
      callable_sig<kotlin_params, kotlin_return>::value;

  /** Human-readable type definition for use as a lambda type. */
  constexpr static std::string_view kotlin_lambda_type =
      callable_sig<kotlin_params, kotlin_lambda_return>::value;
};
...
struct Function<R (*)(Args...)> {
  constexpr static std::string_view signature =
      Function<R(std::decay_t<Args>...)>::signature;
  constexpr static std::string_view kotlin_type =
      Function<R(std::decay_t<Args>...)>::kotlin_type;
};
...
/**
 * Acts as a Java/Kotlin callback proxy in C++ code or a C++ function object
 * proxy in Java/Kotlin code.
 */
template <typename R, typename... Args>
struct ArgType<std::function<R(Args...)>> {
  ...
  constexpr static std::string_view kotlin_function_type =
      "Lkotlin/jvm/functions/Function";
  constexpr static std::string_view semicolon = ";";
  constexpr static std::string_view type_sig =
      join_v<kotlin_function_type, integer_to_digits<sizeof...(Args)>::value,
             semicolon>;
  constexpr static std::string_view kotlin_type =
      Function<R(Args...)>::kotlin_lambda_type;

 private:
  constexpr static std::string_view invoke_sig =
      Function<Object(typename as_object<Args>::type...)>::signature;
 ...
 public:
  static std::function<R(Args...)> native_value(JNIEnv *env, jobject value) {
    ...
    Method invoke = cls.getMethod(
        "invoke", invoke_sig);  // lifecycle bound to object reference
    return [fun = std::move(fun),
            invoke = std::move(invoke)](Args... args) -> R {
      // retrieve an environment reference (which may not be the same as when
      ...
      // Kotlin's `FunctionX` family of classes have an `invoke` method that
      // takes and returns Object instances; primitive types need
      // boxing/unboxing
      if constexpr (!std::is_same_v<R, void>) {
        auto objResult = LocalObjectRef(
            env, env->CallObjectMethod(
                     fun.ref(), invoke.ref(),
                     LocalObjectRef(
                         env, ArgType<Args>::java_box(
                                  env, ArgType<Args>::java_value(env, args)))
                         .ref()...));
        if (env->ExceptionCheck()) {
          throw JavaException(env);
        }
        return ArgType<R>::native_value(
            env, ArgType<R>::java_unbox(env, objResult.ref()));
      } else {
        env->CallVoidMethod(
            fun.ref(), invoke.ref(),
            LocalObjectRef(env, ArgType<Args>::java_box(
                                    env, ArgType<Args>::java_value(env, args)))
                .ref()...);
        if (env->ExceptionCheck()) {
          throw JavaException(env);
  ...
  static jobject java_value(JNIEnv *env,
                            const std::function<R(Args...)> &value) {
    throw std::runtime_error(
        "C++ functions returning a function object to Java/Kotlin are not "
        "supported.");
  }
};
...
L ListArgType<L, T>::native_value(JNIEnv *env, jobject list) {
  ...
  Method getFunc =
      listClass.getMethod("get", Function<Object(int32_t)>::signature);

  L nativeList;
  for (jint i = 0; i < len; i++) {
    LocalObjectRef listElement(env,
                               env->CallObjectMethod(list, getFunc.ref(), i));
    nativeList.push_back(ArgType<T>::native_value(
        env, ArgType<T>::java_unbox(env, listElement.ref())));
  }
...
jobject ListArgType<L, T>::java_value(JNIEnv *env, const L &nativeList) {
  ...
  Method initFunc =
      arrayListClass.getMethod("<init>", Function<void(int)>::signature);
  jobject arrayList =
      env->NewObject(arrayListClass.ref(), initFunc.ref(), nativeList.size());
  Method addFunc =
      arrayListClass.getMethod("add", Function<bool(Object)>::signature);

  for (auto &&element : nativeList) {
    LocalObjectRef arrayListElement(
        env, ArgType<T>::java_box(env, ArgType<T>::java_value(env, element)));
...
S SetArgType<S, E>::native_value(JNIEnv *env, jobject set) {
  ...
  Method iteratorFunc =
      setClass.getMethod("iterator", "()Ljava/util/Iterator;");
  ...
  LocalObjectRef setIterator(env,
                             env->CallObjectMethod(set, iteratorFunc.ref()));

  S nativeSet;
  bool hasNext = static_cast<bool>(
      env->CallBooleanMethod(setIterator.ref(), hasNextFunc.ref()));
  while (hasNext) {
    LocalObjectRef setElement(
        env, env->CallObjectMethod(setIterator.ref(), nextFunc.ref()));
    auto &&element = ArgType<E>::java_unbox(env, setElement.ref());

    nativeSet.insert(ArgType<E>::native_value(env, element));

    hasNext = static_cast<bool>(
        env->CallBooleanMethod(setIterator.ref(), hasNextFunc.ref()));
  }
  return nativeSet;
}
...
jobject SetArgType<S, E>::java_value(JNIEnv *env, const S &nativeSet) {
  ...
  for (auto &&item : nativeSet) {
    LocalObjectRef element(
        env, ArgType<E>::java_box(env, ArgType<E>::java_value(env, item)));
...
M MapArgType<M, K, V>::native_value(JNIEnv *env, jobject map) {
  ...
  Method iteratorFunc =
      entrySetClass.getMethod("iterator", "()Ljava/util/Iterator;");
  ...
  Method getValueFunc =
      entryClass.getMethod("getValue", "()Ljava/lang/Object;");

  LocalObjectRef mapEntrySet(env,
                             env->CallObjectMethod(map, entrySetFunc.ref()));
  LocalObjectRef mapIterator(
      env, env->CallObjectMethod(mapEntrySet.ref(), iteratorFunc.ref()));

  M nativeMap;
  bool hasNext = static_cast<bool>(
      env->CallBooleanMethod(mapIterator.ref(), hasNextFunc.ref()));
  while (hasNext) {
    LocalObjectRef mapEntry(
        env, env->CallObjectMethod(mapIterator.ref(), nextFunc.ref()));
    LocalObjectRef mapKey(
        env, env->CallObjectMethod(mapEntry.ref(), getKeyFunc.ref()));
    LocalObjectRef mapValue(
        env, env->CallObjectMethod(mapEntry.ref(), getValueFunc.ref()));

    auto &&key = ArgType<K>::java_unbox(env, mapKey.ref());
    auto &&value = ArgType<V>::java_unbox(env, mapValue.ref());

    nativeMap[ArgType<K>::native_value(env, key)] =
        ArgType<V>::native_value(env, value);
...
jobject MapArgType<M, K, V>::java_value(JNIEnv *env, const M &nativeMap) {
  ...
  Method putFunc =
      mapClass.getMethod("put", Function<Object(Object, Object)>::signature);
  jobject map =
      env->NewObject(mapClass.ref(), initFunc.ref(), nativeMap.size());
  if (map == nullptr) {
    throw JavaException(env);
  }

  for (auto &&item : nativeMap) {
    LocalObjectRef key(env, ArgType<K>::java_box(
                                env, ArgType<K>::java_value(env, item.first)));
    LocalObjectRef value(env, ArgType<V>::java_box(env, ArgType<V>::java_value(
                                                            env, item.second)));
...
struct JavaOutputBuffer : std::stringbuf {
  JavaOutputBuffer(JNIEnv *env)
      : _env(env),
        _out(LocalClassRef(env, "java/lang/System")
                 .getStaticObjectField("out", "Ljava/io/PrintStream;")),
        _print(LocalClassRef(env, "java/io/PrintStream")
                   .getMethod("print", "(Ljava/lang/String;)V")) {}

  virtual int sync() {
    _env->CallVoidMethod(
        _out.ref(), _print.ref(),
        LocalObjectRef(_env, ArgType<std::string>::java_value(_env, str()))
            .ref());
    str("");
    return 0;
  }
 ...
 * @return A type-safe function pointer to pass to Java's [RegisterNatives]
 * function.
 */
...
struct Adapter {
  ...
  static java_t<result_type> invoke(JNIEnv *env, jclass obj,
                                    java_t<std::decay_t<Args>>... args) {
    try {
...
/**
 * Wraps a native member function pointer into a function pointer callable from
 * Java. Adapts a function with the signature R(T::*func)(Args...).
...
struct MemberAdapter {
  ...
  static java_t<result_type> invoke(JNIEnv *env, jobject obj,
                                    java_t<std::decay_t<Args>>... args) {
    try {
      // look up field that stores native pointer
      LocalClassRef cls(env, obj);
      Field field =
          cls.getField("nativePointer", ArgType<T *>::type_sig.data());
      T *ptr = ArgType<T *>::native_field_value(env, obj, field);

      // invoke native function
      if (!ptr) {
        throw std::logic_error(msg() << "Object " << ArgType<T>::class_name
                                     << " has already been disposed of.");
      }
      if constexpr (!std::is_same_v<result_type, void>) {
        auto &&result = (ptr->*func)(
            ArgType<std::decay_t<Args>>::native_value(env, args)...);
        return ArgType<result_type>::java_value(env, std::move(result));
      } else {
...
/**
 * Wraps a native function pointer or a member function pointer into a function
 * pointer callable from Java.
 * @tparam func The callable function or member function pointer.
 * @return A type-erased function pointer to pass to Java's [RegisterNatives]
 * function.
 */
template <typename T, auto func, typename... Args>
constexpr void *callable(types<Args...>) {
  auto &&f =
      std::conditional_t<std::is_member_function_pointer_v<decltype(func)>,
                         MemberAdapter<T, func, Args...>,
                         Adapter<func, Args...>>::invoke;
  return reinterpret_cast<void *>(f);
}

/**
 * Adapts a constructor function to be invoked from Java on object instantiation
 * with a class method.
 */
...
/**
 * Adapts a destructor function to be invoked from Java when the object is being
 * disposed of. This function is bound to the method close() inherited from the
 ...
 * The Java object holds an opaque pointer to the native object. The lifecycle
 * of the object is governed by Java.
 */
template <typename T>
struct native_class {
  native_class() {
    auto &&bindings = FunctionBindings::value[ArgType<T>::class_name];
    bindings.push_back({"close", Function<void()>::signature, true,
  ...
  native_class &constructor(std::string_view name) {
    static_assert(std::is_function_v<F>,
                  "Use a function signature such as Sample(int, std::string) "
                  "to identify a constructor.");

    auto &&bindings = FunctionBindings::value[ArgType<T>::class_name];
    bindings.push_back({name, Function<F>::signature, false,
  ...
  native_class &function(const char *name) {
    ...
    constexpr bool is_member =
        std::is_member_function_pointer<func_type>::value;

    static_assert(is_free || is_member,
                  "The non-type template argument is expected to be of a free "
                  "function or a compatible member function pointer type.");

    auto &&bindings = FunctionBindings::value[ArgType<T>::class_name];
    bindings.push_back({name, Function<func_type>::signature, is_member,
 ...
 * Data classes marshal data between native and Java code with copy semantics.
...
struct data_class {
  ...
  data_class &field(const char *name) {
    static_assert(std::is_member_object_pointer_v<decltype(member)>,
                  "The non-type template argument is expected to be of a "
                  "member variable pointer type.");
    using member_type = typename FieldType<decltype(member)>::type;

    auto &&bindings = FieldBindings::value[ArgType<T>::type_sig];
    bindings.push_back(
        {name, ArgType<member_type>::type_sig,
         [](JNIEnv *env, jobject obj, Field &fld,
            const void *native_object_ptr) {
           const T *native_object =
               reinterpret_cast<const T *>(native_object_ptr);
           ArgType<member_type>::java_field_value(env, obj, fld,
                                                  native_object->*member);
         },
         [](JNIEnv *env, jobject obj, Field &fld, void *native_object_ptr) {
           T *native_object = reinterpret_cast<T *>(native_object_ptr);
           native_object->*member =
               ArgType<member_type>::native_field_value(env, obj, fld);
         }});
    return *this;
  }
};
...
inline void print_registered_bindings() {
  ...
  os << "/** Represents a class that is instantiated in native code. */\n"
     << "abstract class NativeObject : AutoCloseable {\n"
     << "    /** Holds an opaque reference to an object that exists in the "
  ...
  for (auto &&[class_name, bindings] : FunctionBindings::value) {
    ...
    // class definition
    os << "class " << simple_class_name
       << " private constructor() : NativeObject() {\n";
    ...
    for (auto &&binding : bindings) {
      if (!binding.is_member) {
        os << "        @JvmStatic external fun " << binding.name
           << binding.friendly_signature << "\n";
      }
    }
 ...
 * @param initializer A user-defined function where bindings are registered,
 * e.g. with [native_class].
 */
inline jint java_initialization_impl(JavaVM *vm, void (*initializer)()) {
  ...
  try {
    ...
    for (auto &&[class_name, bindings] : FunctionBindings::value) {
      // find the native class; JNI_OnLoad is called from the correct class
      // loader context for this to work
      LocalClassRef cls(env, class_name.data(), std::nothrow);
      if (cls.ref() == nullptr) {
        java::throw_exception(
            env, msg() << "Cannot find class '" << class_name
                       << "' registered as a native class in C++ code");
        return JNI_ERR;
      }
      ...
      std::transform(bindings.begin(), bindings.end(),
                     ...
                     [](auto &&binding) -> JNINativeMethod {
                       return {const_cast<char *>(binding.name.data()),
                               const_cast<char *>(binding.signature.data()),
                               binding.function_entry_point};
                     });
      jint rc =
          env->RegisterNatives(cls.ref(), functions.data(), functions.size());
      if (rc != JNI_OK) {
        return rc;
      }
    }
    ...
    for (auto &&[class_name, bindings] : FieldBindings::value) {
      ...
      if (cls.ref() == nullptr) {
        java::throw_exception(
            env, msg() << "Cannot find class '" << class_name
                       << "' registered as a data class in C++ code");
        return JNI_ERR;
      }

      // try to look up registered fields
      for (auto &&binding : bindings) {
        jfieldID ref = env->GetFieldID(cls.ref(), binding.name.data(),
        ...
        java::throw_exception(
            env, msg() << "Cannot find field '" << binding.name
                       << "' with type signature '" << binding.signature
                       << "' in registered class '" << class_name << "'");
        return JNI_ERR;
      }
    }
  } catch (std::exception &ex) {
 ...
 */
inline void java_termination_impl(JavaVM *vm) { java::Environment::unload(vm); }

/**
 * Establishes a mapping between a composite native type and a Java data class.
 * This object serves as a means to marshal data between Java and native, and is
 * passed by value.
 */
#define DECLARE_DATA_CLASS(native_type, java_class_qualifier)                \
  ...
  struct ArgType<native_type> : DataClassArgType<native_type> {              \
    ...
    constexpr static std::string_view class_name =                           \
        replace_v<qualified_name, '.', '/'>;                                 \
  };                                                                         \
  }

/**
 * Establishes a mapping between a composite native type and a Java class.
 * This object lives primarily in the native code space, and exposed to Java as
 * an opaque handle.
 */
...
/**
 * Registers the library with Java, and binds user-defined native functions to
 * Java instance and class methods.
 */
#define JAVA_EXTENSION_MODULE()                                     \
  static void java_bindings_initializer();                          \
  JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {           \
    return java_initialization_impl(vm, java_bindings_initializer); \
  }                                                                 \
